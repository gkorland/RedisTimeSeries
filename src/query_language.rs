...
use redis_module::{Context, RedisError, NextArg, RedisString};
...
pub struct CreateCtx {
    ...
    // Label *labels, TODO
    pub options: i16,
    // DuplicatePolicy duplicatePolicy, TODO
    pub skip_chunk_creation: bool,
}
...
// }
impl CreateCtx {
    pub fn parse_create_args(ctx: &Context, mut args: Skip<std::vec::IntoIter<RedisString>>) -> Result<Self, RedisError> {

        let mut ctx = CreateCtx {
            retention_time: 0,  // TODO GlobalConfig.retentionPolicy,
            chunk_size_bytes: 1000, // TODO GlobalConfig.chunkSizeBytes,
            labels_count: 0,
            // labels: None,
            options: 0,
            skip_chunk_creation: false,
        };
        // cCtx->retentionTime = TSGlobalConfig.retentionPolicy;
        // cCtx->chunkSizeBytes = TSGlobalConfig.chunkSizeBytes;
        // cCtx->labelsCount = 0;c
        // cCtx->labels = NULL;
        ...
        while let Ok(arg) = args.next_str() {
            match arg {
                arg if arg.eq_ignore_ascii_case("RETENTION") => ctx.retention_time =  args.next().map_or(Ok(0), |v| v.parse_unsigned_integer())?,
                arg if arg.eq_ignore_ascii_case("CHUNK_SIZE") => ctx.chunk_size_bytes =  args.next().map_or(Ok(0), |v| v.parse_integer())?,
                _ => return Err(RedisError::Str("TSDB: Unknown argument")) // TODO replace right code 
            };
        }

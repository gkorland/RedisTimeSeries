...
use crate::query_language::CreateCtx;
...
pub struct Series{
    ...
    last_value: f64,
    // Label *labels;
    key_name: RedisModuleString,
    labels_count: usize,
    // RedisModuleString *srcKey;
    // const ChunkFuncs *funcs;
    total_samples: usize,
    // DuplicatePolicy duplicatePolicy;
    in_ram: bool // false if the key is on flash (relevant only for RoF)
}

impl Series{
    pub fn new(key_name: RedisModuleString, cCtx: CreateCtx) -> Series{
        let new_series = Series{
            retention_time: cCtx.retention_time,
            chunk_size_bytes: cCtx.chunk_size_bytes,
            options: cCtx.options,
            last_value: 0.0,
            key_name: key_name,
            labels_count: cCtx.labels_count,
            total_samples: 0,
            in_ram: true,
        };
        ...
        // TODO
        // if (!cCtx.skip_chunk_creation) {
        //     Chunk_t *newChunk = newSeries->funcs->NewChunk(newSeries.chunk_size_bytes);

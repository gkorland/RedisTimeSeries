...
use query_language::CreateCtx;
use redis_module::{Context, RedisError, RedisResult, RedisString, REDIS_OK, NotifyEvent, RedisModuleTypeMethods, NextArg};
...
pub const MODULE_NAME: &str = "timeseries";
pub const MODULE_TYPE_NAME: &str = "TSDB-TYPE";

pub const TS_LATEST_ENCVER: i32 = 6;
...
fn info(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
...
// void _TSDB_queryindex_impl(ctx: &Context, QueryPredicateList *queries) {
...
fn query_index(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // RedisModule_AutoMemory(ctx);
...
// static int replyGroupedMultiRange(ctx: &Context,
...
// static int replyUngroupedMultiRange(ctx: &Context,
//                                     RedisModuleDict *result,
//                                     const MRangeArgs *args) {
//     RedisModule_ReplyWithArray(ctx, REDISMODULE_POSTPONED_ARRAY_LEN);
...
// int TSDB_generic_mrange(ctx: &Context, RedisModuleString **argv, int argc, bool rev) {
    // RedisModule_AutoMemory(ctx);
...
fn mrange(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // if (IsMRCluster()) {
    //     int ctxFlags = RedisModule_GetContextFlags(ctx);
...
fn mrevrange(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // if (IsMRCluster()) {
    //     int ctxFlags = RedisModule_GetContextFlags(ctx);
...
fn generic_range(ctx: &Context, args: Vec<RedisString>, rev: bool) -> RedisResult {
//     if (argc < 4) {
//         return RedisModule_WrongArity(ctx);
//     }
...
fn range(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    generic_range(ctx, args, false)
}

fn revrange(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    generic_range(ctx, args, true)
}

// static void handleCompaction(ctx: &Context,
//                              Series *series,
//                              CompactionRule *rule,
...
// static int internalAdd(ctx: &Context,
//                        Series *series,
//                        api_timestamp_t timestamp,
//                        double value,
...
// static inline int add(ctx: &Context,
...
fn madd(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // RedisModule_AutoMemory(ctx);
...
fn create_ts_key(ctx: &Context,
                key_name: &RedisString,
                cCtx: &CreateCtx,
                key: &RedisKeyWritable) -> Option<Series>{
    // if (*key == NULL) {
...
fn create(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // RedisModule_AutoMemory(ctx);
    ...
    let create_ctx = CreateCtx::parse_create_args(ctx, args)?;

    let key = ctx.open_key_writable(&key_name);

    let key_value = key.get_value::<Series>(&REDIS_TIMESERIES_TYPE)?;

    if key_value.is_some() {
        return Err(RedisError::Str("TSDB: key already exists"));
    }
    ...
    let series = create_ts_key(ctx, &key_name, &create_ctx, &key);
    // RedisModule_CloseKey(key);
...
fn alter(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // RedisModule_AutoMemory(ctx);
 ...
 */
fn delete_rule(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // RedisModule_AutoMemory(ctx);
...
*/
fn create_rule(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // RedisModule_AutoMemory(ctx);
...
*/
fn incrby(ctx: &Context, args: Vec<RedisString>) -> RedisResult {
    // RedisModule_AutoMemory(ctx);
...
fn mget(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // if (IsMRCluster()) {
    //     int ctxFlags = RedisModule_GetContextFlags(ctx);
...
// static inline bool verify_compaction_del_possible(ctx: &Context,
//                                                   const Series *series,
...
fn delete(ctx: &Context, args: Vec<RedisString>)  -> RedisResult {
    // RedisModule_AutoMemory(ctx);
...
// void FlushEventCallback(ctx: &Context, RedisModuleEvent eid, uint64_t subevent, void *data) {
...
// void swapDbEventCallback(ctx: &Context, RedisModuleEvent e, uint64_t sub, void *data) {
...
// void keyAddedToDbDict(ctx: &Context,
...
// int keyRemovedFromDbDict(ctx: &Context,
//                          RedisModuleString *key,
//                          void *value,
//                          int swap_key_metadata,
//                          int writing_to_swap) {
...
// void persistCallback(ctx: &Context, RedisModuleEvent eid, uint64_t subevent, void *data) {
...
// void ShardingEvent(ctx: &Context, RedisModuleEvent eid, uint64_t subevent, void *data) {
//     /**
...
fn notify_callback(_ctx: &Context, _event_type: NotifyEvent, event: &str, key: &str) {
...
// void ReplicaBackupCallback(ctx: &Context,
...
pub static REDIS_TIMESERIES_TYPE: RedisType = RedisType::new(
    MODULE_TYPE_NAME,
    TS_LATEST_ENCVER,
    RedisModuleTypeMethods {
        version: redis_module::TYPE_METHOD_VERSION,

        rdb_load: None, // TODO Some(series_rdb_load),
        rdb_save: None,  // TODO Some(series_rdb_save),
        aof_rewrite: None, // TODO add support RMUtil_DefaultAofRewrite
        free: None, // Some(FreeSeries), TODO1

        // Currently unused by Redis
        mem_usage: None, // TODO Some(SeriesMemUsage),
        digest: None,
        ...
        free_effort: None,
        unlink: None,
        copy: None, // Some(CopySeries), TODO
        defrag: None,
    },
);
...
redis_module! {
    name: "timeseries",
    version: 99_99_99,
    data_types: [REDIS_TIMESERIES_TYPE],
    commands: [
        ["ts.create", create, "", 0, 0, 0],
        ["ts.alter", alter, "", 0, 0, 0],
        ["ts.createrule", create_rule, "", 0, 0, 0],
        ["ts.deleterule", delete_rule, "", 0, 0, 0],
        ["ts.add", add, "", 0, 0, 0],
        ["ts.incrby", incrby, "", 0, 0, 0],
        ["ts.decrby", incrby, "", 0, 0, 0], // TODO ??
        ["ts.range", range, "", 0, 0, 0],
        ["ts.revrange", revrange, "", 0, 0, 0],

        ["ts.queryindex", query_index, "readonly", 0, 0, -1],
        ...
        ["ts.madd", madd, "readonly", 1, -1, 3],
        ["ts.mrange", mrange, "readonly", 0, 0, -1],
        ["ts.mrevrange", mrevrange, "readonly", 0, 0, -1],
        ["ts.mget", mget, "readonly", 0, 0, -1],
    ],
    event_handlers : [
        // @TRIMMED TODO
        [@GENERIC @SET @STRING @EVICTED @EXPIRED @LOADED: notify_callback]
    ]
}
...
*/
// int RedisModule_OnLoad(ctx: &Context, RedisModuleString **argv, int argc) {
    ...
    // initGlobalCompactionFunctions(); // TODO
